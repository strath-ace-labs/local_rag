...
from ragas.testset.utils import load_as_score, parse_regex
...
class TestsetGenerator:
    ...
    def _filter_context(self, context: str) -> bool:
        ...
        results = self.critic_llm.generate(prompts=[prompt], max_new_tokens= 5)
    ...
    def _seed_question(self, context: str) -> str:
        ...
        prompt = ChatPromptTemplate.from_messages([human_prompt])
        results = self.generator_llm.generate(prompts=[prompt], suffix ="Question: ", stop_words= ['?"', '?','?"\n\n','?"\n'])
        print("Creating QUESTION")
        print(results)
        results = parse_regex(results.generations[0][0].text.strip(), r'Question:\s*([^\n]+)')
        #print("Creating QUESTION")
        #print(results)
    ...
    def _filter_question(self, question: str) -> bool:
        ...
        pattern = r'^\{.*?"reason":\s*"(.*?)",\s*"verdict":\s*"(.*?)"\}'
        pattern = r"^\{*[^\n]+\}"
        pattern = r'^Result:\s*([^\n]+})'
        pattern = r'^Result:\s*([\s\S]*?\})'
        #print("Filtering Question: ")
        ...
        while counter < 3: 
            if json_results == {}:
                try:
                    results = self.critic_llm.generate(prompts=[prompt], suffix ="Result: ", stop_words= ['Yes', 'No'])
                    results = results.generations[0][0].text.strip()
                    results = results+'"}'
                    #print(results)
                    results = parse_regex(results, pattern)
    ...
    def _question_transformation(self, prompt, question: str) -> str:
        ...
        prompt = ChatPromptTemplate.from_messages([human_prompt])
        results = self.generator_llm.generate(prompts=[prompt])
        results = parse_regex(results.generations[0][0].text.strip(), r'^(.*)')
        return results
    ...
    def generate(
        ...
        while count < test_size and available_nodes != []:
            ...
            if is_valid_question:
                ...
                print(question)
                samples.append(
                        #DataRow(qstn, [ctx], [ans], evolve_type, episode_done, text_chunk, score)
                        {"question": question, "context": context, "answer": answer, "evolve_type": evolve_type, "text_chunk": text_chunk, "score": score, "metadata": nodes[0].metadata}
                    )
                ...
                if count % 10 == 0: 
                    print("Generated {} samples".format(count))
                    
                pbar.update(count)
            #break
        
        return samples#TestDataset(test_data=samples)
